          ...
          subscription = characteristic.value.listen((data){
            ...
            for (int i=0; i <data.length; i+=2){
              if (data.length-i > i+2){
                ...
                if (FFAppState().OscopeIndex * 1000 / 3300 > FFAppState().SampleTime) {
                  FFAppState().OscopeIndex = 0; // Reset index for overwrite
                }
                if (FFAppState().OscopeTime.isNotEmpty) {
                  // Update existing index if time exceeds sampleTime
                  FFAppState().OscopeVoltage[FFAppState().OscopeIndex] = voltage;
                  FFAppState().OscopeTime[FFAppState().OscopeIndex] = FFAppState().OscopeIndex * 1000 / 3300; // 3300 Hz
                } 
                else {
                  // Add new data point
